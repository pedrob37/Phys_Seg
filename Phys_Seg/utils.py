...
import os
from Phys_Seg.paths import folder_with_parameter_files


def get_params_fname(sequence, physics_flag):
    if physics_flag:
        model_name = f"{sequence}_phys_NSS.pth"
    else:
        model_name = f"{sequence}_base_NSS.pth"
...
def maybe_download_parameters(sequence='mprage', physics_flag=True, force_overwrite=True):
    ...
    :param physics_flag: True or False
    ...
    out_filename = get_params_fname(sequence=sequence, physics_flag=physics_flag)
...
# TBC
# Is this method necessary in my case?
